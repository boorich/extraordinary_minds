...
const EVALUATION_QUESTIONS = [
  {
    round: 1,
    question:
      "Before we begin your evaluation, tell me - what drives you to seek a position aboard this vessel?",
    context: "Opening question to assess motivation and communication clarity",
  },
  {
    round: 2,
    question:
      "Interesting. Now, describe a complex problem you've navigated and how you charted your course through it.",
    context: "Technical and problem-solving assessment",
  },
  {
    round: 3,
    question:
      "Your approach reveals much. When faced with conflicting objectives, how do you determine your trajectory?",
    context: "Decision-making and ethical reasoning assessment",
  },
  {
    round: 4,
    question:
      "We traverse uncertain waters. Share an instance where you adapted to unexpected changes in your environment.",
    context: "Adaptability and resilience assessment",
  },
  {
    round: 5,
    question:
      "Final question: How would you contribute to our collective journey through the digital seas?",
...
${this.character.style.all.join("\\n")}`;
  ...
  async evaluateResponse(
    ...
    if (!input.trim() || input.length < 10) {
      return {
        content:
          "Your response lacks substance. A crew member must communicate with clarity and purpose.",
        isValid: false,
        evaluationScore: 0,
    ...
    const hasConcreteness = /specific|example|instance|case|when|how/i.test(
      input
    );
    ...
    this.currentEvaluationScore =
      this.evaluationHistory.reduce((a, b) => a + b, 0) /
...
If the response quality is below ${
              this.failureThreshold
            }, be stern but constructive in your criticism.
...
End with a relevant follow-up question that builds on their response.`,
      ...
      return {
        ...
        evaluationScore: score,
        failureReason:
          score < this.failureThreshold
            ? "Response below acceptable threshold"
            : undefined,
      };
    }
  }
...
Format each option as a brief phrase that could be selected by the user.`,
  ...
  private determineOptionType(
    option: string
  ): "technical" | "philosophical" | "creative" | "analytical" {
    const patterns = {
...
Include: Commentary on potential role aboard the vessel`;
  ...
  getFailureReason(): string {
    ...
    const lowScores = this.evaluationHistory.filter(
      (score) => score < this.failureThreshold
    ).length;
    if (lowScores > 2)
      return "Multiple responses failed to meet minimum standards";
    if (this.currentEvaluationScore < this.failureThreshold)
      return "Overall evaluation score below threshold";
  ...
  private determineStrengths(): string[] {
    const strengths = [];
    if (this.currentEvaluationScore > 0.7)
      strengths.push("clear communication");
    if (this.evaluationHistory[1] > 0.7)
    ...
    if (this.evaluationHistory[4] > 0.7)
      strengths.push("collaborative potential");
    return strengths;
  }
}
